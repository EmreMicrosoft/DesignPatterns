...
std::unordered_map<std::string, std::function<bool()>> contracts() {
    return {
        ...
        {"exception-tracking", [] { struct ExceptionTracker { std::vector<std::string> reports; void record(const std::string& service, const std::exception& error) { reports.push_back(service + ":" + error.what()); } }; ExceptionTracker tracker; try { throw std::runtime_error{"catalogue unavailable"}; } catch (const std::exception& error) { tracker.record("catalogue-api", error); } return tracker.reports == std::vector<std::string>{"catalogue-api:catalogue unavailable"}; }},
        {"log-deployments-and-changes", [] { struct ChangeLog { std::vector<std::string> entries; void recordDeployment(const std::string& version) { entries.push_back("deployed:" + version); } }; ChangeLog log; log.recordDeployment("2026.08.13"); return log.entries == std::vector<std::string>{"deployed:2026.08.13"}; }},
        {"server-side-page-fragment-composition", [] { struct PageComposer { std::string compose(const std::string& header, const std::string& body) const { return "<page>" + header + body + "</page>"; } }; return PageComposer{}.compose("<header>catalogue</header>", "<main>patterns</main>") == "<page><header>catalogue</header><main>patterns</main></page>"; }},
        {"client-side-ui-composition", [] { struct Dashboard { std::string compose(const std::unordered_map<std::string, std::string>& fragments) const { return fragments.at("catalogue") + " | " + fragments.at("status"); } }; return Dashboard{}.compose({{"catalogue", "patterns:262"}, {"status", "ready"}}) == "patterns:262 | ready"; }},
